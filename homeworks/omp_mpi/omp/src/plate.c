...
uint8_t init_plate(plate_t *plate, char *jobFilePath, char *subBin,
  ...
  if (sscanf(line_readed, "%s %" SCNd64 " %lf %lf %lf", BinaryFile,
    ...
    plate->arrayM1 =
        array_to_matrix(plate->plateM1, plate->rows, plate->columns);
    plate->arrayM2 =
        array_to_matrix(plate->plateM2, plate->rows, plate->columns);
    plate->time = Time;
