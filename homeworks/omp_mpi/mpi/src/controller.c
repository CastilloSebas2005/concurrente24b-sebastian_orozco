...
void init_controller(char *argv[], int rank, int rank_count) {
  ...
  uint64_t states_to_report[linesToRead];
  MPI_Status status;
  ...
  if (rank == 0) {
    ...
    while (work_done < linesToRead) {
      ...
      MPI_Recv(&completed_task, 1, MPI_UINT64_T, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &status);
      work_done++;
      uint64_t received_state;
      MPI_Recv(&received_state, 1, MPI_UINT64_T, status.MPI_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      states_to_report[completed_task] = received_state;
      if (work_to_do < linesToRead) {
        MPI_Send(&work_to_do, 1, MPI_UINT64_T, status.MPI_SOURCE, 0, MPI_COMM_WORLD);
        work_to_do++;
      } else {
        uint64_t no_more_work = linesToRead;
        MPI_Send(&no_more_work, 1, MPI_UINT64_T, status.MPI_SOURCE, 0, MPI_COMM_WORLD);
      }
    }
    ...
    for (size_t i = 0; i < (size_t)linesToRead; i++) {
      if (plates[i].blankline == 0) {
        ...
        char* line = make_line_to_report(plates[i].lineReaded, time, states_to_report[i]);
    ...
    while (true) {
      MPI_Recv(&work_to_do, 1, MPI_UINT64_T, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      if (work_to_do >= linesToRead) {
        break;
      }
      ...
      if (plates[work_to_do].blankline == 0) {
        ...
        printf("Plate number %li calculated correctly by rank, with this number of states %li: %i\n", work_to_do, states , rank);
