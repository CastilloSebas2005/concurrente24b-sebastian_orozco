...
/// This is the limit to make the simulation
...
/// @var shared_data::task_mutex
/// This is the mutex to protect the task
/// @var shared_data::next_row
...
uint64_t init_simulation(plate_t plate, uint64_t Thread_count) {
  if (plate.columns > 50 && plate.rows > 50 && Thread_count > 1) {
    ...
    shared_data->next_row = 1;
    // Initialize the barrier
    init_barrier(&shared_data->barrier, Thread_count + 1);
    pthread_mutex_init(&shared_data->task_mutex, NULL);
    ...
    // Indicate that threads should terminate
    ...
    pthread_mutex_destroy(&shared_data->task_mutex);
    free(shared_data);
    free(threads);
    free(private_data);
    return states;
...
void *transfer_thread(void *private_data) {
  ...
  while (true) {
    ...
    // Initial static assignment of rows
    uint64_t rows_per_thread = (shared_data->R - 2) / shared_data->thread_count;
    uint64_t start_row = 1 + data->thread_id * rows_per_thread;
    uint64_t end_row = (data->thread_id == shared_data->thread_count - 1)
                           ? shared_data->R - 1
    ...
    // Dynamic assignment of remaining rows
    while (true) {
      pthread_mutex_lock(&shared_data->task_mutex);
      uint64_t row = shared_data->next_row++;
      pthread_mutex_unlock(&shared_data->task_mutex);

      if (row >= shared_data->R - 1) {
        break;
      }

      for (uint64_t j = 1; j < shared_data->C - 1; j++) {
        uint64_t idx = row * shared_data->C + j;
        double up = shared_data->matrix2[idx - shared_data->C];
        double down = shared_data->matrix2[idx + shared_data->C];
        double left = shared_data->matrix2[idx - 1];
        double right = shared_data->matrix2[idx + 1];
