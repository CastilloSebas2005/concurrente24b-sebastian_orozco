...
void *golpear_pinata(void *data) {
    ...
    uint64_t base_golpes = shared_data->cantidad_golpes / shared_data->thread_count;
    uint64_t golpes_restantes = shared_data->cantidad_golpes % shared_data->thread_count;
    ...
    if (shared_data->cantidad_golpes == 0) {
        printf("Thread %" PRIu64 "/%" PRIu64 " %" PRIu64 " hits, broke the pinata\n",
